# VERSION MODIFICADA PARA CY — v4 (Corrección de path Unity Catalog y Regex)
"""
CAMBIOS v4:
- spark.read.format("text").option("wholetext", "true") en lugar de binaryFile.
- Se utiliza col("_metadata.file_path") para evitar el bug de duplicación de rutas.
- Corrección en Regex de Facility para evitar que capture "Notes" o "Numbers".
- Corrección en Regex de Geo para que no trunque direcciones como "885".
"""
import os
import re
import logging
...
class CoyoteExtractor(BaseExtractor):
    ...
    def _extract_geo(self, block: str) -> dict:
        ...
        # Caso A: CITY, ST ZIP
        geo_m = re.search(r'^(.*?)\s+([A-Za-z\s\-\.]+?),\s*([A-Z]{2})\s+(\d{5}(?:-\d{4})?)', zone, re.I)
        if geo_m:
            addr_raw = geo_m.group(1).strip()
            city_raw = geo_m.group(2).strip()
            
            # Arreglo para que números de dirección no trunquen la captura (Bug "885")
            city_words = city_raw.split()
            if len(city_words) > 2:
                 result["city"] = " ".join(city_words[-2:])
                 result["address"] = (addr_raw + " " + " ".join(city_words[:-2])).upper()
            else:
                 result["city"] = city_raw
                 result["address"] = addr_raw.upper()

            result["state"]   = geo_m.group(3).strip().upper()
            result["zipcode"] = geo_m.group(4).strip()
            return result
