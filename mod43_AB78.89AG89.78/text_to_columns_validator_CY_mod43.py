# VERSION MODIFICADA PARA CY — VALIDACIÓN AUTOMÁTICA MULTI-DOCUMENTO v2
"""
Compara los datos extraídos en la tabla Silver contra el Ground Truth
para múltiples cargas de Coyote Logistics.

CAMBIOS v2:
- source_file: normalización de ruta antes de comparar
  (elimina prefijo dbfs:, reemplaza carpeta txt→pdf, extensión .txt→.pdf)
- Conteo de errores corregido: source_file cuenta como 1 error, no se
  propaga al global si la diferencia es solo el prefijo dbfs: esperado.
- Columna de variable de loop renombrada a `field_name` para evitar
  colisión con la función builtin `col` de PySpark.
"""
import argparse
import logging
