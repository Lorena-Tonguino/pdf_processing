...
class CoyoteExtractor(BaseExtractor):
    ...
    def _extract_all_stops(self, raw_text: str) -> list:
        ...
        while i < len(parts) - 1:
            ...
            # Ignorar bloques colapsados sin Address — son artefactos del conversor PDF
            if not re.search(r'\bAddress\b', block, re.I):
                i += 2
                continue
    ...
    def _extract_facility(self, block: str) -> str:
        ...
        raw = re.sub(r'(?i)\s*Driver\s+Work\b', ' ', raw).strip()       # Reemplaza con espacio para no unir palabras
    ...
    def _extract_geo(self, block: str) -> dict:
        #
        # Inicializa el resultado con campos vacíos
        result = {"address": "", "city": "", "state": "", "zipcode": ""}

        # Busca la zona de dirección entre la etiqueta "Address" y los delimitadores
        # conocidos (Contact, Phone, Facility Notes, etc.)
        addr_zone_m = re.search(
            r'Address\s+(.+?)(?=\s+(?:Contact|Phone|Facility\s+Notes|STACEY|FRCO|$))',
            block, re.I | re.S
        )
        # Si no encuentra la etiqueta Address en el bloque, retorna vacío
        if not addr_zone_m:
            return result

        # Extrae el texto capturado entre Address y el delimitador
        zone = addr_zone_m.group(1)

        # Limpia el zone eliminando etiquetas operacionales que no forman
        # parte de la dirección real
        zone = re.sub(r'(?i)\bSLIC\b', '', zone)        # Código de bodega
        zone = re.sub(r'(?i)\bN/A\b', '', zone)          # No aplica
        zone = re.sub(r'(?i)Driver\s+Work', '', zone)    # Instrucción operacional
        zone = re.sub(r'(?i)No\s+Touch', '', zone)       # Instrucción operacional
        zone = re.sub(r'(?i)Lumper', '', zone)            # Instrucción operacional
        ...
        if geo_m:
            before_comma = geo_m.group(1).strip()  # Todo antes de la coma
            state        = geo_m.group(2).strip().upper()
            zipcode      = geo_m.group(3).strip()

            # Intento 1: usa SLIC o N/A como delimitador natural entre dirección y ciudad.
            # Cuando aparecen entre la dirección y la ciudad, todo antes es dirección
            ...
            if slic_m:
                ...
                # Limpia SLIC y N/A que puedan quedar como prefijo de la ciudad
                ...
                road_m = re.search(
                    r'^(.+?\b(?:RD|DR|AVE|ST|BLVD|HWY|WAY|CT|PKWY|LN|PL|TER|CIR|LOOP|RTE|SQ|FWY|BYP)\b\.?)\s+([A-Za-z].+)$',
                    before_comma, re.I
                )
                if road_m:
                    # El texto después de la abreviatura vial puede contener
                    # información adicional de la dirección (SUITE, BUILDING)
                    # seguida de la ciudad
                    after_road = road_m.group(2).strip()

                    # Extrae la ciudad como el sufijo final de tokens que empiezan
                    # con mayúscula (cubre McDonough, Lithia Springs, Plant City, etc.)
                    # sin capturar patrones tipo "BUILDING A" que son parte de la dirección
                    city_m = re.search(r'((?:[A-Z]\S+)(?:\s+(?:[A-Z]\S+))*)$', after_road)
                    if city_m:
                        city = city_m.group(1).strip()
                        # Recupera el sufijo de dirección que queda entre la abreviatura
                        # vial y el inicio de la ciudad (ej: "SUITE 230", "BUILDING A")
                        city_pos    = after_road.rfind(city)
                        addr_suffix = after_road[:city_pos].strip() if city_pos > 0 else ""
                        address     = (road_m.group(1).strip() + (" " + addr_suffix if addr_suffix else "")).strip().upper()
                    else:
                        # Si no encuentra ciudad, usa todo after_road como ciudad
                        address = road_m.group(1).strip().upper()
                        city    = after_road
                else:
                    # Intento 3 (fallback): toma la última palabra como ciudad
                    # y el resto como dirección
        ...
        # ── CASO B: ZIP separado del estado (ej: "Ciudad, GA N/A 30122") ─────
        # Cubre formatos donde el zip no está inmediatamente después del estado
        geo_m2 = re.search(r'([A-Za-z][A-Za-z\s\-\.]+?),\s*([A-Z]{2})\b.*?(\d{5}(?:-\d{4})?)', zone, re.I)
        if geo_m2:
            city_raw   = geo_m2.group(1).strip()
            city_words = city_raw.split()
            # Toma la última palabra del grupo capturado como ciudad
            city       = city_words[-1] if city_words else city_raw
            # La dirección es todo lo que aparece antes del inicio de la ciudad
        ...
        # Si ningún patrón matchea, retorna resultado vacío
        return result
